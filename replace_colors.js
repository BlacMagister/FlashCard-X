...
const colorMap = {
  ...
  'text-white': 'text-[var(--color-card-bg)]',
...
files.forEach(file => {
  ...
  for (const [oldClass, newClass] of Object.entries(colorMap)) {
    // Need to use regex to avoid partial matches? No, tailwind classes are space-separated or quote-separated.
    // We can just do a global replace for now, it's specific enough.
    content = content.split(oldClass).join(newClass);
  }
  
  // Custom manual replacements for specific colors used in style objects or specialized classes
  // e.g. text-[#FDFCF8] (inverse color)
  content = content.replace(/text-\[\#FDFCF8\]/g, 'text-[var(--color-app-bg)]');
  
  // challenge special bg
